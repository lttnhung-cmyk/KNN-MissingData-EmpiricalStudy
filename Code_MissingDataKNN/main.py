...
def knnIC_approach(x_train_missing, x_test_missing, y_train, y_test, random_state=None):
  ...
  x_train_impute, x_test_impute = impute_knn(x_train_missing, x_test_missing, best_n_neighbors)
...
def impute_knn(fit_data, transform_data, number_of_neighbors):
  imputer = KNNImputer(n_neighbors=number_of_neighbors)
  fit_data_imputed = imputer.fit_transform(fit_data)
  transform_data_imputed = imputer.transform(transform_data)
  return fit_data_imputed, transform_data_imputed
...
def find_best_n_neighbors(x_train_missing, y_train, random_state=None):
  ...
  for n_neighbors in range(1, MAX_TUNING_NEIGHBORS+1):
      x_actual_train_imputed, x_validation_imputed = impute_knn(x_actual_train, x_validation, n_neighbors)
...
def meanIC_approach(x_train_missing, x_test_missing, y_train, y_test, random_state=None):
  ...
  t1 = time.perf_counter()
  x_train_impute, x_test_impute = impute_mean(x_train_missing, x_test_missing)
  train_time = time.perf_counter() - t1
...
def impute_mean(fit_data, transform_data):
...
def find_best_n_neighbors_mean(x_train_missing, y_train, random_state=None):
  ...
  x_actual_train_imputed, x_validation_imputed = impute_mean(x_actual_train, x_validation)
...
def generate_missing_values(original_data, missing_rate):
  ...
  missing_id = np.random.choice(original_data.size, n_missing, replace=False)
...
def get_Micromass_data():
  ...
  print(df.head())
  X = df.to_numpy()
  # lọc feature gần-hằng-số (variance ~0) được thực hiện trong get_split_standardized,
  # tính trên train của từng trial để tránh leak — ở đây trả về đủ toàn bộ cột thô.
...
def get_split_standardized(trial_seed):
    x_train, x_test, y_train, y_test = train_test_split(features_x, labels_y, test_size=0.2, random_state=trial_seed)

    if DATASET_NAME == "Micromass":
        # Micromass có nhiều feature gần-hằng-số (variance ~0); lọc theo variance
        # tính trên train của trial này để tránh leak thông tin từ test.
        var_train = np.var(x_train, axis=0)
        valid_cols = var_train > 1e-5
...
if __name__ == '__main__':
    ...
    # step 1: lấy data (chưa chuẩn hóa — StandardScaler sẽ fit riêng trên train của từng trial trong get_split_standardized)
    features_x, labels_y = DATASETS[DATASET_NAME]()

    # step 2: chạy thí nghiệm đầy đủ (nhiều missing rate x nhiều trial) để lấy kết quả cho paper
    run_experiment(methods, get_split_standardized,
    ...
    summary = summarize_results(df)  # bảng mean±std theo missing_rate, kèm n_neighbors_mean/mode
    print(summary[["method", "missing_rate", "accuracy_mean", "accuracy_sem",
                    "n_neighbors_mean", "n_neighbors_mode",
                    "total_time_sec_mean", "n"]].to_string())
