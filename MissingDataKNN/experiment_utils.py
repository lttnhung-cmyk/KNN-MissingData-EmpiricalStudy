...
def run_experiment(methods: dict, get_split_func, missing_rates: list,
    ...
    get_split_func: callable(seed) -> (x_train, x_test, y_train, y_test) dữ liệu gốc chưa missing
    """
...
def generate_missing_values(original_data, missing_rate, rng: np.random.RandomState):
    ...
    missing_id = rng.randint(0, original_data.size, round(missing_rate * original_data.size))
...
def summarize_results(df: pd.DataFrame, metrics=("accuracy", "total_time_sec")) -> pd.DataFrame:
    grouped = df.groupby(["method", "missing_rate"])
